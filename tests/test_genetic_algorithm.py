...
from diagnostico_saude_mulher.genetic_algorithm.engine import ESPACO_BUSCA, GeneticConfig, HyperparameterGeneticOptimizer, Individual
...
class GeneticAlgorithmTests(unittest.TestCase):
    ...
    def test_populacao_inicial_respeita_espaco_busca(self) -> None:
        ...
        for individual in population:
            for nome, valor in individual.genes.items():
                self.assertIn(valor, ESPACO_BUSCA[nome])

    def test_crossover_gera_filho_valido(self) -> None:
        parent1 = Individual(genes={nome: valores[0] for nome, valores in ESPACO_BUSCA.items()})
        parent2 = Individual(genes={nome: valores[-1] for nome, valores in ESPACO_BUSCA.items()})
    ...
    def test_mutacao_mantem_dominio_dos_genes(self) -> None:
        original = Individual(genes={nome: valores[0] for nome, valores in ESPACO_BUSCA.items()})
